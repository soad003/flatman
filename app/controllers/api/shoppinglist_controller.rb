class Api::ShoppinglistController < Api::RestController
  around_action :wrap_in_transaction, only: [:create, :destroy, :delete_checked, :was_shopping]
  ...
  def create
    ...
    flat.save!
    Newsitem.createShoppinglist(@sl, current_user) unless @sl.is_private?
  end

  def destroy
    sl = Shoppinglist.find_list_with_user_constraint(params[:id], current_user)
    Newsitem.deleteShoppinglist(sl, current_user) unless sl.is_private?
    sl.destroy!
    respond_with(nil)
  end
  ...
  def delete_checked
    ...
    sl.shoppinglistitems.select(&:checked).each(&:destroy!)
    respond_with(nil)
  end
