class Newsitem < ActiveRecord::Base
  ...
  def self.createShoppinglist(shoppinglist, user)
    Newsitem.saveNewsitem(user,
                          Newsitem::CATEGORIES[:shoppinglist],
  ...
  def self.clearShoppingListID(shoppinglist, _user)
    Newsitem.where(key: shoppinglist.id,
                   category: Newsitem::CATEGORIES[:shoppinglistitem])
            .update_all(key: nil)
  end

  def self.createShoppinglistitem(shoppinglistitem, user)
    newsitem = Newsitem.getNewsitem(shoppinglistitem.shoppinglist.id,
                                    Newsitem::CATEGORIES[:shoppinglistitem],
                                    Newsitem::ACTIONS[:add],
                                    user)
    if newsitem.nil?
      Newsitem.saveNewsitem(user,
                            Newsitem::CATEGORIES[:shoppinglistitem],
                            Newsitem::ACTIONS[:add],
                            shoppinglistitem.shoppinglist.id,
  ...
  def self.clearTodoListID(todolist, _user)
    Newsitem.where(key: todolist.id,
                   category: Newsitem::CATEGORIES[:todolistitem])
            .update_all(key: nil)
  end

  def self.createTodolistitem(todolistitem, user)
    newsitem = Newsitem.getNewsitem(todolistitem.todo.id,
                                    Newsitem::CATEGORIES[:todolistitem],
                                    Newsitem::ACTIONS[:add],
                                    user)
    if newsitem.nil?
      Newsitem.saveNewsitem(user,
  ...
  def self.getNewsitem(key, category, action, user)
    Newsitem.find_by(key: key,
                     category: category,
                     user: user,
                     flat: user.flat,
                     action: action,
                     updated_at: (DateTime.current - 10.minutes)..DateTime.current)
  end

  def self.createMessage(text, user)
    newsitem = Newsitem.where(category: Newsitem::CATEGORIES[:message],
                              flat: user.flat,
                              action: Newsitem::ACTIONS[:add],
                              updated_at: (DateTime.current - 20.seconds)..DateTime.current)
  ...
  def self.createComment(text, newsitem_id, user)
    comment = Newsitem.saveNewsitem(user,
                                    Newsitem::CATEGORIES[:comment],
                                    Newsitem::ACTIONS[:add],
                                    nil,
                                    text,
                                    newsitem_id)
  ...
  def self.deleteUser(user)
    Newsitem.saveNewsitem(user,
                          Newsitem::CATEGORIES[:useraction],
                          Newsitem::ACTIONS[:remove],
                          nil,
                          nil,
                          nil)
  ...
  def self.push(newsitem)
    if Newsitem.sendPush(newsitem)
      newsitem.user.flat.users.each do |mate|
        ...
        next unless mate.id != newsitem.user.id &&
                    mate.pushflag &&
                    !(mate.device_token == '' || mate.device_token.nil?)
  ...
  def self.sendPush(newsitem)
    if newsitem.isShopping && newsitem.action == Newsitem::ACTIONS[:done] # shopping done
      lastnewsitem = Newsitem.getNewsitemForPush(newsitem.flat,
                                                 [Newsitem::CATEGORIES[:shoppinglist]],
                                                 [Newsitem::ACTIONS[:done]])
    elsif newsitem.isShopping && (newsitem.action != Newsitem::ACTIONS[:done]) # last shopping
      lastnewsitem = Newsitem.getNewsitemForPush(newsitem.flat,
                                                 [
                                                   Newsitem::CATEGORIES[:shoppinglist],
                                                   Newsitem::CATEGORIES[:shoppinglistitem]
  ...
  def self.getNewsitemForPush(flat, categories, actions)
    if actions.nil?
      return Newsitem.where(category: categories,
                            flat: flat,
                            updated_at: (DateTime.current - 2.minutes)..DateTime.current)
                     .order(updated_at: :desc).first
    else
      return Newsitem.where(category: categories,
                            action: actions,
